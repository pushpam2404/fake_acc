...
Intelligence-grade fake account, botnet, impersonator, and social threat detection engine built for **ITBP / Ministry of Home Affairs (SIH-1775)**. Features dual-platform XGBoost classification (101,000+ accounts), real-time SHAP explainability, platform-specific Playwright session injection, local PyTorch neural NLP (`all-MiniLM-L6-v2`) semantic threat detection, interactive SVG coordinated threat network topology, and 1-click forensic PDF case file export across **X**, **Instagram**, and **Facebook**.
...
├── backend/
│   ├── main.py                # FastAPI REST server, URL router, Multimodal Fusion & CORS
│   ├── schemas.py             # Pydantic data contracts (PostMediaItem, ContentAnalysis, AnalyzeResponse)
│   ├── session_manager.py     # Local Playwright storageState cookie manager (0 passwords stored)
│   ├── facebook_scraper.py    # Facebook script-tag structured data parser & zero-login scraper
│   ├── twitter_scraper.py     # X Playwright session-injected scraper (followers, bio, tweets)
│   ├── instagram_scraper.py   # Instagram Playwright mobile emulation & Instaloader session fallback
│   ├── playwright_scraper.py  # Platform scraping dispatcher & orchestrator
│   ├── content_analyser.py    # Local SentenceTransformer neural NLP, cosine matrix, threat taxonomy
│   ├── osint_scraper.py       # OSINT profile extraction & graceful fallback
...
├── feature_extractor/
│   ├── build_twitter.py       # X 13-feature ETL & ratio engineering
│   └── build_meta.py          # Meta 7-feature ETL & ratio engineering
├── frontend/                  # React 18 + TypeScript + Vite Dashboard
│   ├── src/
│   │   ├── App.tsx            # Main Dashboard (URL scanner, manual form, live forensic report)
│   │   ├── MediaAuditView.tsx # Live Scraped Post Gallery, Caption Uniformity & Threat Badges
│   │   ├── NetworkGraph.tsx   # Interactive SVG force-directed threat topology graph
│   │   ├── SessionManager.tsx # Platform session & authentication management panel
│   │   ├── api.ts             # Axios client with session status/capture endpoints
│   │   ├── presets.ts         # Verified telemetry profiles for X and Meta
│   │   └── types.ts           # TypeScript interfaces matching backend schemas
│   ├── package.json           # React frontend dependencies
│   └── vite.config.ts         # Vite dev server configuration
├── models/
│   ├── saved/                 # Tuned model binaries (twitter_xgboost_tuned.pkl, meta_xgboost_tuned.pkl)
│   └── predict.py             # Dual-platform predictor & continuous SHAP explainability engine
...
- **X Model (13 Features, 90.04% Accuracy)**: `followers`, `following`, `post_count`, `verified`, `description_length`, `account_age_days`, `follower_following_ratio`, `reputation_score`, `username_length`, `digits_in_username`, `digit_ratio_username`, `has_url`, `posts_per_day`.
...
### 2. Platform-Specific Scraping & Session Management
- **Zero Credentials Stored**: Password-free session cookie persistence via local Playwright `storageState` JSON files (`backend/sessions/`).
- **X (Twitter)**: Session-injected Playwright Chromium automation extracting follower counts, following, tweet history, verified badges, and description text.
- **Instagram**: Headless mobile emulation with automatic overlay dismissal + Instaloader session cookie fallback.
- **Facebook**: Zero-login `<script type="application/json">` / `ld+json` DOM structured data extraction.

### 3. Zero-Shot Neural NLP & Phishing Engine (SentenceTransformer)
...
### 4. Universal Identity Discrepancy & Impersonation Engine
...
### 5. Interactive SVG Coordinated Threat Topology Graph
- **NetworkX** graph theory calculations: Degree Centrality, Graph Density, and Clique Count.
- **Full Drag & Drop Interactivity**: SVG matrix transformations (`getScreenCTM().inverse()`) enabling fluid 1:1 node dragging without drift.
- **Live Auto-Arrange Physics**: Spring-relaxation simulation to untangle node clusters dynamically.
- **Color-Coded Tooltips & Zoom/Pan**: Sleek top overlay cards distinguishing organic mutual connections (green/teal) from botnets and C2 relays (crimson/amber).

### 6. Official ITBP / MHA Forensic PDF Export
...
| :--- | :--- | :---: | :---: | :---: | :---: |
| **X (Twitter)** | Tuned XGBoost | 64,919 Accounts | **90.04%** | **85.63%** | 0.18s |
...
### 2. Single Account Telemetry Inference
```bash
...
### 4. Platform Session Management
```bash
# Check connected sessions
GET http://localhost:8000/session/status

# Launch visible browser to capture session
POST http://localhost:8000/session/capture
Content-Type: application/json
...
POST http://localhost:8000/session/revoke
Content-Type: application/json
